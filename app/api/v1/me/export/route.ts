...
export async function POST(request: Request) {
  ...
  const [profile, favorites, reading, journeys, memories, conversations] = await Promise.all([
    database.select().from(databaseSchema.userProfiles).where(eq(databaseSchema.userProfiles.userId, userId)),
    database.select().from(databaseSchema.favorites).where(eq(databaseSchema.favorites.userId, userId)),
    database.select().from(databaseSchema.readingProgress).where(eq(databaseSchema.readingProgress.userId, userId)),
    database.select().from(databaseSchema.journeyProgress).where(eq(databaseSchema.journeyProgress.userId, userId)),
    database.select().from(databaseSchema.memoryItems).where(eq(databaseSchema.memoryItems.userId, userId)),
    database.select().from(databaseSchema.conversations).where(eq(databaseSchema.conversations.userId, userId)).orderBy(asc(databaseSchema.conversations.createdAt)),
  ]);
