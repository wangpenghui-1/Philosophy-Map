...
210人物版本选择原覆盖矩阵中的 `batch-01` 至 `batch-06`，共180名新增人物；与原有30人合并后形成210人公开版本。六个批次均满足跨地区与跨时代约束，生产任务存放在：
...
只有 `content/knowledge/people`、`concepts`、`traditions` 等正式实体目录中的 `published` 记录才进入网站。生产任务不会进入搜索索引、3D地球或详情页。`coverage/release-210.json` 记录180个生产候选与正式人物ID的可追溯映射。
...
```bash
# 首次生成或补齐六个批次的缺失任务；不会覆盖已经推进的任务状态
npm run content:batch:prepare

# 验证六个批次的manifest和独立任务是否稳定
...
队列和报告写入 `artifacts/production/<batch-id>/`，不进入版本库。工作者结果必须说明自动工作者身份；失败结果必须包含错误码、说明和是否可重试。研究任务本身始终不能写入 `published`；210人本地发布候选由独立的确定性装配步骤生成，并保留自动审核身份。
...
`ready-for-promotion` 只表示研究任务通过装配门禁。为便于最终审核，210人版本在当前发布分支中生成可见的 `published` 实体，但这不代表外部同行评审、合并 `main` 或线上部署；这些动作仍需用户最终批准。
