...
export default async function MediaPage() {
  ...
  return <>
    <header className={styles.pageHeader}><div><span className={styles.eyebrow}>MEDIA LIBRARY</span><h1>媒体资产</h1><p>统一管理人物图像、插图、文档与音频的文件、授权、真实性和替代文本。</p></div><Link className={styles.primaryAction} href="/admin/media/new">上传媒体</Link></header>
    <section className={styles.panel}><div className={styles.panelHeading}><h2>{rows.length} 个媒体资产</h2><span>{principal.mode === "local-preview" ? "只读公开快照" : "对象存储与数据库"}</span></div>
