...
一个以风格化3D地球为叙事入口、以仓库化内容为核心的世界哲学知识库。当前成品公开213位人物，覆盖古代至当代、八个主要思想区域，并可沿人物、概念、传统和文本继续阅读。
...
- 213人发布清单、6个跨区域生产批次与覆盖率报告
- 213份完整媒体卡片：开放许可照片、历史或传统形象，以及明确标注的艺术化形象

新增人物记录以学术百科、区域哲学史与大学出版社文集为主要二级来源；自动审核身份与外部同行评审明确区分。每次修复或更新通过一次统一审核后，由受保护流程自动完成合并、版本发布和部署；非published内容不会被自动提升。
...
- `/journeys`：八条思想旅程目录；`/journey/[journeyId]`：共享旅程播放器。

原有 `/explore`、`/story/[chapter]` 和人物slug保持可用。`/explore?question=<id>`、`/explore?thinker=<slug>` 等深链接可恢复对应的地球探索状态。
...
`npm run content:audit`会阻止无效引用、缺失端点、不合格发布状态和不满足内容层级的条目进入生成物。`npm test`会先生成发布构建，再检查知识库路由、213人物发布清单、旧关系保留、候选隔离、规模模拟、媒体政策、关系语义和客户端分包预算。`npm run test:e2e`使用桌面与移动端 Chromium 检查核心交互、知识库URL恢复、降级模式和视觉快照。
